...
function receiptMatches(packet, filter) {
  if (filter === "active") return !packet.archiveReady;
  if (filter === "all") return true;

  const status = String(packet.resolutionStatus || packet.title || "").toLowerCase();

  if (filter === "approved") return status.includes("approved") && !packet.archiveReady;
  if (filter === "rejected") return status.includes("rejected") && !packet.archiveReady;
  if (filter === "returned") return status.includes("returned") && !packet.archiveReady;
  if (filter === "resolved") return status.includes("resolved") && !packet.archiveReady;
