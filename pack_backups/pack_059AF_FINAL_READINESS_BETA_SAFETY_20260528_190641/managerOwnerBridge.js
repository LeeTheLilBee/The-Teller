...
export function getClearManagerDecisionCopy(status = "Reviewed") {
  const clean = String(status || "Reviewed").toLowerCase();

  if (clean.includes("approved")) {
    return {
      headline: "Approved",
      employeeMessage: "Approved. Your request has been reviewed and accepted by the manager.",
      nextStep: "No action is needed from you unless the manager asks for something else.",
      tone: "approved",
    };
  }

  if (clean.includes("needs proof") || clean.includes("needs info") || clean.includes("more")) {
    return {
      headline: "Needs more info",
      employeeMessage: "The manager needs more information before this can be approved.",
      nextStep: "Open the response and add the missing details or proof.",
      tone: "needs-proof",
    };
  }
  ...
  if (clean.includes("rejected") || clean.includes("declined")) {
    return {
      ...
      employeeMessage: "This request was not approved. Review the manager note before submitting again.",
      nextStep: "You can add a clearer follow-up if the manager left room for correction.",
      tone: "rejected",
    };
  }

  if (clean.includes("upward") || clean.includes("owner") || clean.includes("tower")) {
    return {
      headline: "Sent upward",
      employeeMessage: "Your request was sent upward for owner or Tower review.",
      nextStep: "No extra action is needed right now unless someone asks for more information.",
      tone: "upward",
    };
  }
  ...
  return {
    headline: "Reviewed",
    employeeMessage: "Your request has been reviewed.",
    nextStep: "Check the response details for next steps.",
    tone: "reviewed",
  };
}
